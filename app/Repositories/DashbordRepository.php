...
//------------Doctor-wise-token-function----------------    
    public function getNextTokenByPriorityDoctor(Department $department, $user_id, $counter_id)
    {
        return $department->queues()
                    ->where('called', 0)
                    ->where('created_at', '>', Carbon::now()->format('Y-m-d 00:00:00'))
...
public function getTodayQueueByCounterDoctor($department_id = '', $user_id = '', $counter_id = '')
    {
        return Queue::whereBetween('created_at', [Carbon::now()->format('Y-m-d').' 00:00:00', Carbon::now()->format('Y-m-d').' 23:59:59'])
                    ->where('called', 0)
                    ->where('department_id', $department_id)
    ...
    public function getTodayPalatinumpDoctor($department_id = '', $id = '', $counter_id = '')
    {
        return Queue::whereBetween('created_at', [Carbon::now()->format('Y-m-d').' 00:00:00', Carbon::now()->format('Y-m-d').' 23:59:59'])
                    ->where('called', 0)->where('priority', 1)
                    ->where('department_id', $department_id)
